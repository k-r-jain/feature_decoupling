...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, random_sized_crop=False):
        ...
        if self.dataset_name=='imagenet':
            assert(self.split=='train' or self.split=='validation')
    ...
    def __len__(self):
        return int(len(self.data) * 0.05) # TODO
    ...
    def get_iterator(self):
        ...
        if self.unsupervised:
            ...
            def _load_function(idx):
                ...
                img, cls_labels, index = self.dataset[idx]
                rotation_labels = torch.LongTensor([0, 1, 2, 3])
                image_indices = torch.LongTensor([index, index, index, index])
                return img, rotation_labels, image_indices, cls_labels
            def _collate_fun(batch):
                batch = default_collate(batch)
                # assert(len(batch)==3) # TODO
